...
alias (n,m);
scalar
F0      Nominal Frequency /60/
Fmin    nadir Frequency /59/
f_db    Deadband Freq   /0.03/
td       governer response /1/
tnadir  nadir time /0.5/
Stiff    stifness /2/;
parameters
W_max(k) Renewables installed capacity
                         /k1 = 400, k2 = 400/;
...
L(d) Load level /d1 600
                 d2 400/

V(d) value of lost load /d1 00
                         d2 0  /
...
TABLE Generation_Para (i,*)  generators input data
        Pmax      C       Rmax   CPFR    CSFR    LTRR    STRR   ramp_rate inertia reserveMax droop
i1      600      25       0      10      5       2       0.5    3          3        0        .05
i2      400      36       80     2       1       1       0.25   3          3.5      0        .04;
...
*[sum(i,Generation_Para(i,'CSFR')*SFR(i,s))]
...
generation_max,flow_RT,flow_max_RT,shedding,spillage,slack_RT,slack_DA,PFreqResponsemin,PFreqResponsemax,delafreq,PFRresv,SecFreqResup,SecFreqRes,spillageup,Sheddingup;

ObjectiveFunction..                      cost=e=sum(s,Prob(s)*{[sum(i,Generation_Para(i,'C')*PowerAdj(i,s))+[sum(i,Generation_Para(i,'CPFR')*PFR(i,s))]
                                                 +[sum(i,Generation_Para(i,'CSFR')*SFR(i,s))]]+[sum(d,V(d)*Load_shed(d,s))]})
                                                 +[sum(i,Generation_Para(i,'C')*Pgen_DA(i))];
...
*LowerLmt(i,s)..                          PowerAdj(i,s)=g=0;

generation_min(i,s) ..                   [Pgen_DA(i)+PowerAdj(i,s)-PFR(i,s)]=g=0;

generation_max(i,s) ..                   [Pgen_DA(i)+PowerAdj(i,s)+PFR(i,s)+SFR(i,s)]=l=Generation_Para(i,'Pmax');


PFRresv(i,s)..                           PFR(i,s) =l=  Generation_Para(i,'reserveMax') ;
*PFRresvmin(i,s)..                        PFR(i,s) =g=  0;
PFreqResponsemax(i,s)..                  PFR(i,s) =l=  Generation_Para(i,'STRR') * tnadir;

PFreqResponsemin(s)..                     sum(i,PFR(i,s))=g= sum(k,Power_spill(k,s)) + sum(d,Load_shed(d,s));

SecFreqRes(i,s)..                       SFR(i,s)=l= 10*Generation_Para(i,'Ramp_Rate');

delafreq(i,s)..                          delf(i,s)=e= -1*(sum(k,Power_spill(k,s)) - sum(d,Load_shed(d,s))) /( (sum(d,L(d))/f0)*((1/Generation_Para(i,'Droop')+(Stiff))) );

SecFreqResup(s)..                        sum(i,SFR(i,s)) =g= -1* Sum(i,(delf(i,s))*(1/Generation_Para(i,'Droop'))+(sum(k,Power_spill(k,s)) + sum(d,Load_shed(d,s))));
...
sheddingup(d,s)..                          Load_shed(d,s)=g=0;

spillage(k,s)..                          Power_spill(k,s)=l=Renewable_Para(k,s);

spillageup(k,s)..                          Power_spill(k,s)=g=0;
...
display
SFR.l,PFR.l,delf.l,cost.l,weightedcostspill, costnew, LambdaN_DA, LambdaN_RT, Rnew_DA.l, Pgen_DA.l, PowerAdj.l, Power_spill.l, Load_shed.l,Spill_T;
